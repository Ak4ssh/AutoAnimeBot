...
from RyzenApi.gogoanimeapi import gogoanime as gogo
import HelpFile.formating_results as format
from config import bot
from HelpFile.helper_functions import *
...
class Anime():
    ...
    async def callback_for_download(event):
        ...
        if int(x[1]) < 101:
            ...
            if int(x[1]) % 100 == 0:
                ...
                button2[current_row].append(Button.inline(
                    f'{num_of_buttons}01 - {x[1]}', data=f'etz:{x[1]}:{x[1]}'))
            await event.edit(
                buttons=button2
            )
    ...
    async def callback_for_download_long(event):
        ...
        for i in range(int(x[3])):
            button2[current_row].append(Button.inline(
                str(i+1), data=f'spp:{i+1}:{x[1]}:{x[1]}'))
            if (i+1) % 5 == 0:
                button2.append([])
                current_row = current_row + 1
        await event.edit(
